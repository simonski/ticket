...
help:
	@printf "Available targets (run any; granular sub-targets exist in the Makefile):\n\n"
	@printf "Build & run:\n"
	@printf "  make setup            Install dev dependencies (Go + Node + Playwright).\n"
	@printf "  make build            Build ./bin/tk (does not change the version).\n"
	@printf "  make build-linux      Cross-build a linux/amd64 binary into ./bin/tk-linux.\n"
	@printf "  make caddy            Local HTTPS reverse proxy https://ticket.localhost.\n"
	@printf "  make dev              Print env vars for local development mode.\n"
	@printf "  make clean            Remove built binaries from ./bin.\n\n"
	@printf "Test & lint:\n"
	@printf "  make test             Fast default: unit tests.\n"
	@printf "  make test-fast        Developer loop: unit + Go API smoke.\n"
	@printf "  make test-api         CLI/API contract tests.\n"
	@printf "  make test-browser     Browser suite (site2, live UI).\n"
	@printf "  make test-browser-full  Full browser end-to-end suite.\n"
	@printf "  make test-all         Everything: unit + api + browser + docs/harness.\n"
	@printf "  make test-go-cover    Go tests with per-package coverage thresholds.\n"
	@printf "  make lint             golangci-lint + gosec.\n"
	@printf "  make vulncheck        govulncheck on all Go packages.\n\n"
	@printf "CI (mirrors GitHub Actions):\n"
	@printf "  make ci               ci-verify + ci-browser.\n"
	@printf "  make ci-verify        Verify job: openapi + coverage + build + lint + vulncheck.\n"
	@printf "  make ci-browser       Browser job.\n"
	@printf "  make ci-bootstrap     Install deps for the CI parity flow.\n\n"
	@printf "Release & deploy:\n"
	@printf "  make publish          Bump version, build + push Docker images to GHCR.\n"
	@printf "  make release          Full release: binaries + checksums + SBOM + formula + tap.\n"
	@printf "  make docker           Build the local Docker image.\n"
	@printf "  make docker-up        Start deploy/compose.yaml (uses deploy/.env).\n"
	@printf "  make docker-down      Stop deploy/compose.yaml.\n"
	@printf "  make deploy           Copy the deploy bundle to the EXE_DEV_URL host.\n\n"
	@printf "Granular test-*, release-*, ci-bootstrap-*, and *-bin sub-targets exist for\n"
	@printf "composition and CI; see the Makefile for the full list.\n\n"
