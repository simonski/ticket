...
func TestTeamUpdateAndDelete(t *testing.T) {
	...
	team, err := CreateTeam(db, "Alpha", nil)
	if err != nil {
		t.Fatalf("CreateTeam() error = %v", err)
	}

	updated, err := UpdateTeam(db, team.ID, "Beta", nil)
	if err != nil {
	...
	if err := DeleteTeam(db, team.ID); err != nil {
...
func TestUpdateTeamWithParentCycleProtection(t *testing.T) {
	...
	root, err := CreateTeam(db, "Root", nil)
	if err != nil {
		t.Fatalf("CreateTeam(Root) error = %v", err)
	}
	child, err := CreateTeam(db, "Child", &root.ID)
	if err != nil {
		t.Fatalf("CreateTeam(Child) error = %v", err)
	}

	// Cannot be its own parent
	if _, err := UpdateTeam(db, root.ID, "", &root.ID); err == nil {
		t.Fatal("UpdateTeam(self-parent) error = nil, want error")
	}

	// Cannot create cycle: root -> child -> root
	if _, err := UpdateTeam(db, root.ID, "", &child.ID); err == nil {
		t.Fatal("UpdateTeam(cycle) error = nil, want error")
	}

	// Valid update: set child's parent to nil
	updated, err := UpdateTeam(db, child.ID, "Renamed Child", nil)
	if err != nil {
	...
	kept, err := UpdateTeam(db, child.ID, "", nil)
	if err != nil {
...
func TestTeamMemberRemoveAndList(t *testing.T) {
	...
	team, err := CreateTeam(db, "Gamma", nil)
	if err != nil {
		t.Fatalf("CreateTeam() error = %v", err)
	}
	alice, err := CreateUser(db, "alice", "password123", "user")
	...
	if _, err := AddTeamMember(db, team.ID, alice.ID, TeamRoleMember, "Dev"); err != nil {
		t.Fatalf("AddTeamMember(alice) error = %v", err)
	}
	if _, err := AddTeamMember(db, team.ID, bob.ID, TeamRoleOwner, "Lead"); err != nil {
	...
	role, found, err := TeamRoleForUser(db, team.ID, alice.ID)
	if err != nil {
	...
	if err := RemoveTeamMember(db, team.ID, alice.ID); err != nil {
...
func TestTeamAgentRemoveAndList(t *testing.T) {
	...
	team, err := CreateTeam(db, "Delta", nil)
	if err != nil {
		t.Fatalf("CreateTeam() error = %v", err)
	}
	agent, _, err := CreateAgent(db, "secret")
	...
	if _, err := AddTeamAgent(db, team.ID, agent.ID); err != nil {
		t.Fatalf("AddTeamAgent() error = %v", err)
	}

	agents, err := ListTeamAgents(db, team.ID)
	if err != nil {
	...
	if err := RemoveTeamAgent(db, team.ID, agent.ID); err != nil {
...
func TestProjectTeamMemberRemoveAndList(t *testing.T) {
	...
	team, err := CreateTeam(db, "Epsilon", nil)
	if err != nil {
		t.Fatalf("CreateTeam() error = %v", err)
	}

	if _, err := AddProjectTeamMember(db, project.ID, team.ID, ProjectRoleViewer); err != nil {
		t.Fatalf("AddProjectTeamMember() error = %v", err)
	}

	members, err := ListProjectTeamMembers(db, project.ID)
	if err != nil {
	...
	if err := RemoveProjectTeamMember(db, project.ID, team.ID); err != nil {
...
func TestTeamDescendantIDs(t *testing.T) {
	...
	grandchild, err := CreateTeam(db, "Grandchild", &child.ID)
	if err != nil {
		t.Fatalf("CreateTeam(Grandchild) error = %v", err)
	}

	descendants, err := TeamDescendantIDs(db, root.ID)
	if err != nil {
